...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || t.registerErrorDefault);
      }
      ...
      <div className="w-full max-w-md mx-auto my-auto py-8">
        <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? t.registeringText : t.registerSubmitBtn}
            </button>
          </form>

          <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 text-center">
            <span className="text-xs text-slate-500">{t.hasAccountText} </span>
            <Link
              href="/login"
              onClick={() => sounds.playClick()}
